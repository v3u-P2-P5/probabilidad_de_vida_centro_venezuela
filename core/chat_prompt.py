...
def system_prompt_v2(lang: str, now_iso: str = "") -> str:
    """Contrato del asistente agéntico (v3): persona cálida + reglas estrictas.

    El texto del prompt vive en core/chat_persona.py (PROMPT_ES/PROMPT_EN), con
    tono empático-profesional y TODAS las reglas anti-alucinación integradas; aquí
    solo se inserta la hora actual. Todo dato factual debe venir de una skill.
    """
    from core.chat_persona import PROMPT_EN, PROMPT_ES
    when = f" Hora actual (UTC): {now_iso}." if now_iso else ""
    base = PROMPT_EN if lang == "en" else PROMPT_ES
    return base.replace("{when}", when)
